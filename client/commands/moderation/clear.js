    ...
    try {
        ...
        try {
            message.channel.bulkDelete(amount).then(() => {
                message.channel.send(`Cleared ${args[0]} messages`).then((msg) => msg.delete({ timeout: 1000 }));
            });
        } catch (e) {
        ...
        if (guild.modLogChannel !== "" || guild.modLogChannel !== null || guild.modLogChannel !== undefined) {
            ...
            let embed = new MessageEmbed()
                ...
                .setDescription(
                    `Bulk Delete in ${message.channel}, ${args[0]} messages deleted by **${message.author.username}#${message.author.discriminator}**`
                )
                .setColor("#FF4D9C")
                .setTimestamp();
            channel.send(embed);
        ...
        message.channel.send(
            `Unable to show clear log due to an error. If encountered, please send to developers. (!support to get invite link) \n\`[${moment().utc()}] Clear Log | ${e}\``
        );
    }
};
